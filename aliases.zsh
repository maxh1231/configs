...
alias filediff="git log --no-merges --color --format=' * %C(red)%h%C(reset) - %s %C(green)(%ar)%C(reset) %C(bold brightblue)<%an>%C(reset)' | fzf --ansi --print-query --preview 'git diff-tree --no-commit-id --name-only -r $(echo {2})'"