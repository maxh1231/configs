...
alias gld='git log --no-merges --color --format=" * %C(red)%h%C(reset) - %s %C(green)(%ar)%C(reset) %C(bold brightblue)<%an>%C(reset)" | fzf --ansi --preview "git diff-tree --no-commit-id --name-only -r {2}" | awk "{print \$2}" | xargs git diff'
